// Seed QA: Residencial Las Palmas — via Supabase JS client (ESM)
...
const E = 'aaaaaaaa-aaaa-aaaa-eeee-000000000001'
...
const rec = (mes, n) => `cccccccc-cccc-cccc-${mes}-00000000000${n}`
const p = (mes, n) => `dddddddd-dddd-dddd-${mes}-00000000000${n}`
...
async function run() {
  ...
  const unidadNums = [101,102,103,201,202,203,301,302]
  for (const mes of meses) {
    for (let i = 1; i <= 8; i++) {
      const venc = mes.prefix === '0401' && i >= 7 ? '2026-04-30' : mes.venc
      recibosData.push({
        id: rec(mes.prefix, i), condominio_id: C, unidad_id: u(String(unidadNums[i-1])),
